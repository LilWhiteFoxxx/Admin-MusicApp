...
// data placeholder
import categories_management from "@db/categories";
...
const CategoryManagementTable = () => {
  ...
  const getQty = (category) => {
    if (category === "all") return categories_management.length;
    return categories_management.filter(
  ...
  const dataByStatus = () => {
    if (category === "all") return categories_management;
    return categories_management.filter(
      (product) => product.status === category
    );
  };
